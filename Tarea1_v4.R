# ÚLTIMA EDICIÓN POR: ALVARO CARRIZO
# PENÚLTIMA EDICIÓN POR: YAKU VALDIVIA
# FECHA Y HORA APROZ ULTIMA EDICION: 7 Abril 13.37
...
# ^ no es necesario filtrar por >0 de precipitación?
# nop, corriendo la línea de abajo se ve que no existen valores negativos
#sum(df_balances$ETr<0)
#sum(df_balances$Precipitacion<0)
summary(df_balances)

svg(here("graficos", "graficos_PP_Etr_deltaS.svg"), width = 11, height = 10)

par(mfrow = c(3, 1)) #3 filas; 1 columna
par(mar = c(5, 4, 3, 1)) #bottom, left, top, right
par(oma = c(3,0,0,0)) #generación de un margen global, para así generar un label de X global
...
mtext("Fecha", side = 1, outer = TRUE, line = 1)
...
# TT.text(                     #ETIQUETAS DE LOS HORIZONTES, eliminar # para agregar etiquetas
#  tri.data = text1,
#  geo = geo1,
