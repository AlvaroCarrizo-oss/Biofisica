...
for (i in seq_along(horizontes)) {
  ...
  plot(alm_plot,
       ...
       col = paleta_alm)
  plot(vect(st_transform(polygon, crs(alm_plot))), add = TRUE, border = "black", lwd = 1.5)
  dev.off()
}
...
plot(alm_total_proj,
     ...
     col = paleta_alm)
plot(vect(st_transform(polygon, crs(alm_total_proj))), add = TRUE, border = "black", lwd = 1.5)
dev.off()
...
maxs <- sapply(horizontes, function(hz) global(almacenamiento_cut[[hz]], "max", na.rm = TRUE)[1,1])
...
for (hz in horizontes) {
  ...
  plot(
    alm_proj,
    main = paste("Almacenamiento (cm agua) -", hz, "cm"),
    col = paleta_alm,
    range = c(min_alm_hz, max_alm_hz)
  )
  plot(vect(st_transform(polygon, crs(alm_proj))), add = TRUE, border = "black", lwd = 1.2)
}
...
area_pixel_m2 <- res_m[1] * res_m[2]  # área por píxel en m²
